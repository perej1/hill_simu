...
%%
\thesisabstract{
Assessment of the tail behavior is important for example in risk management. However, analyzing the tail is challenging since there is very little data available on extremes. One possible solution to the problem is extreme value theory which provides mathematical theory for analyzing tail behavior. In extreme value theory there is a parameter called extreme value index that describes the thickness of the tail and evaluation of the extreme value index is often of high practical importance. One of the possible estimators for the extreme value index is the Hill estimator that is one of the most popular estimators for heavy-tailed distributions. This thesis work reviews the asymptotic properties of the Hill estimator. The focus is on the consistency. First we review the connection between heavy tailed distributions and regularly varying functions that play a crucial role in proof of the consistency. Then we review the results for consistency and for asymptotic normality. Finally, finite sample properties of the Hill estimator are examined with Monte Carlo simulation.
}
...
\supervisor{Apulaisprof.  Pauliina Ilmonen}
...
In both Figures \ref{pareto} and \ref{cauchy} it seems that with small sample sizes variance decreases rapidly but as the sample size increases the decrease of the variance slows down. This can be seen by looking at the quartiles. At the beginning the quartiles squeeze around the median but eventually after about $n=12500$ it is hard to see any further approach to the median.

It seems that in Figure \ref{cauchy} the median is a little further from $\gamma$ than in Figure \ref{pareto}. This suggests that for Cauchy distribution larger sample size is needed for an accurate estimate for $\gamma$ than for Pareto distribution. In both Figures \ref{pareto} and \ref{cauchy} quartiles are equally far away from median which suggests that asymptotic distribution of the Hill estimator might be symmetrical.
